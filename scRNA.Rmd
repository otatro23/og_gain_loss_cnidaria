...
need_cNMF <- FALSE
need_SAMap <- FALSE
run_feature_plot <- TRUE
run_heatmaps <- FALSE
run_variance_plots <- TRUE
...
```{r setup_focal_nodes}
load("../orthodollo/og4dollo.Rdata")
rm(ogdf)

# view the tip nums to compute gains at tips 
tip_nums <- tr1[["tip.label"]]
tip_nums

tr2<-read.tree("../orthodollo/cnidarian_timetree.tre") 
root_node <- getMRCA(tr2, c("Mnemiopsis", "Lampea", "Coeloplana"))
tr2<-read.tree("../orthodollo/cnidarian_timetree.tre") %>% phytools::reroot(., node.number = root_node ,position=0.01) %>% ladderize() 

tr2<-phytools::rescale(tr2,model="depth",
    depth=650)
bt <- branching.times(tr2)
bt <- data.frame(node_time = as.numeric(names(bt)), age = as.numeric(bt))

node_labs <- read.csv("../orthodollo/cnid_node_labels copy.csv")
...
node_ages <- merge(node_labs, bt, by = "node_time") %>% 
  merge(focal_nodes, by.x = "node_number", by.y = "node") %>% 
  select(name, age)
...
feature_plots <- function(sc_obj, ogs_gained, ogmap, node, bins = 24, seurat = TRUE, reduction, color_norm = TRUE, min = 0, max = 1, border = "black") {
  ...
  if (color_norm) {
    ...
    feature_plot <- 
      FeaturePlot(seurat_obj, features = "node_gains1", reduction = reduction) + 
...
plots <- function(seurat_obj, ogmap, nodes, bins = 24, reduction, ref, genus, cell_types, features = TRUE, heat = TRUE, varplot = TRUE){
  ...
  if(features) {
    ...
    for (node in nodes[1:length(nodes) - 1]){
      print(node)
    ...
    for (node in nodes){
      ...
      try({plot_output <- feature_plots(seurat_obj, ogs_gained = og_gain, ogmap = ogmap, node = node, bins = 16, seurat = TRUE, reduction = reduction, color_norm = TRUE, min = min, max = max, border = node_cols[node])
    ...
    feature_plot <- cowplot::plot_grid(plotlist = c(clusters, plot_color_fix, legend), nrow = 1, rel_widths = c(1.5, rep(1, length(plot_color_fix)), 0.05))
    
    pdf(file.path("/Users/oliviatatro/Downloads/surf/figures", genus, paste0(genus, "_scRNA_feature_plots.pdf")), height = 4, width = (length(nodes) + 1.5 + 0.05)*4)
    print(feature_plot)
    dev.off()
  }
  ...
  if (heat){
    ...
    for (node in nodes){
      ...
      try(print(heatmaps(ref, og_gain, node, ogmap = ogmap, deepfri_met, threshold = 0.5))) 
  ...
  if (varplot) {
    ...
    try({genes <- var_plot(seurat_obj, ogmap, nodes)})
...
DefaultAssay(Tr) <- "RNA"
if (!file.exists(file.path(dir,"cNMF", "matrix.mtx")) & need_cNMF){
...
if (!file.exists(file.path(dir,"SAMap", "matrix.mtx")) & need_SAMap) {
...
# -------------------------------------RUN PLOTS-------------------------------
nodes <- c("cnid", "endomed", "med", "hyd", "turritopsis")
...
Tr_feature <- plots(Tr, aurelia_map, nodes, bins = 16, "tsne", turritopsis_ref, genus = "turritopsis", cell_types = "cell_type", features = run_feature_plot, heat = run_heatmaps, varplot = run_variance_plots)
...
if (file.exists(file.path(dir, "seurat_aurelia.Rds"))) {
  ...
  Au <- subset(Au, subset = nFeature_RNA > 200 & nFeature_RNA < 2500)
  ...
  Au <- RunPCA(Au)
  Au <- RunUMAP(Au, dims = 1:15)
  Au <- RunTSNE(Au, dims = 1:15)
...
# -------------------------------------RUN PLOTS-------------------------------
nodes <- c("cnid", "endomed", "med", "acr", "scyph", "aurelia")

dir.create(file.path("/Users/oliviatatro/Downloads/surf/figures", "turritopsis"))

# we will be using the RNA assay for the featureplots because it has all the genes, but the expression needs to be normalized
DefaultAssay(Au) <- "RNA"
Au <- NormalizeData(Au, normalization.method = "LogNormalize")

# seurat object used aurelia gene ids 
Au_feature <- plots(Au, aurelia_map, nodes, bins = 16, "tsne", aurelia_ref, genus = "aurelia", cell_types = "cell_type", features = run_feature_plot, heat = run_heatmaps, varplot = run_variance_plots)
...
acropora_map$sc_ID <- gsub("_", "-", acropora_map$sc_ID)


# ------------------------------ RUN PLOTS --------------------------------

nodes <- c("cnid", "anth", "anth2", "hex", "scler", "acropora")
genus <- "acropora"
dir.create(file.path("/Users/oliviatatro/Downloads/surf/figures", genus))
...
Am_feature <- plots(Am, acropora_map, nodes, bins = 16, "tsne", acropora_ref, genus = "acropora", cell_types = "cell_type", features = run_feature_plot, heat = run_heatmaps, varplot = run_variance_plots)
...
# ------------------------------------RUN PLOTS -----------------------------------
nodes <- c("cnid", "anth", "anth2", "hex", "act", "nematostella")

# we will be using the RNA assay for the featureplots because it has all the genes, but the expression needs to be normalized
DefaultAssay(Nv) <- "RNA"
Nv <- NormalizeData(Nv, normalization.method = "LogNormalize")

Nv_feature <- plots(Nv, nematostella_map, nodes, bins = 16, "umap", nematostella_ref, genus = "nematostella", cell_types = "IDs", features = run_feature_plot, heat = run_heatmaps, varplot = run_variance_plots)
...
# ------------------------------------RUN PLOTS--------------------------------
nodes <- c("cnid", "endomed", "med", "hyd", "hydra")
...
# seurat object used aurelia gene ids 
Hv_feature <- plots(Hv, hydra_map, nodes, bins = 16, "tsne", hydra_ref, genus = "hydra", cell_types = "cell_type", features = run_feature_plot, heat = run_heatmaps, varplot = run_variance_plots)
...
# ------------------------------------RUN PLOTS--------------------------------
nodes <- c("cnid", "anth", "oct", "xenia")
dir.create(file.path("/Users/oliviatatro/Downloads/surf/figures", "xenia"))

# seurat object used aurelia gene ids 
Xe_feature <- plots(Xe, xenia_map, nodes, bins = 16, "tsne", xenia_ref, genus = "xenia", cell_types = "cell_ids", features = run_feature_plot, heat = run_heatmaps, varplot = run_variance_plots)
