...
var_plot <- function(seurat_obj, ogmap, nodes) {
  ...
  genes <- data.frame(gene = names(var), var = var, mean = mean)
  ...
  label_y <- var_lims[2]
  p <- ggplot() +
    geom_point(data = genes, x = mean, y = var, size = 0.5) +
    scale_x_continuous(limits = mean_lims) +
    scale_y_continuous(limits = var_lims) +
    xlab("Mean of log expression") +
    ylab("Variance of log expression") +
    theme_bw() +
    annotate("text", x = 0, y = label_y, label = "All genes", size = 5, color = "black", hjust = 0)
  label_y = label_y - 0.1
...
```{r plots}
plots <- function(seurat_obj, ogmap, nodes, bins = 24, reduction, ref, genus){
  ...
  print("Normalizing RNA assay...")
  DefaultAssay(seurat_obj) <- "RNA"
  NormalizeData(seurat_obj, normalization.method = "LogNormalize")
  ...
  for (node in nodes){
    ...
    plot_output <- feature_plots(seurat_obj, ogs_gained = og_gain, ogmap = ogmap, node = node, bins = 16, seurat = TRUE, reduction = "umap", color_norm = TRUE, min = min, max = max, border = node_cols[node])
  ...
  pdf(file.path("/Users/oliviatatro/Downloads/surf/figures", genus, paste0(genus, "_scRNA_feature_plots.pdf")), height = 8, width = length(nodes) * 4)
  cowplot::plot_grid(plotlist = all_plots, nrow = 2, ncol = length(nodes) + 1)
  dev.off()
...
if (file.exists(file.path(dir, "turritopsis_medusa.Rds"))) {
  ...
  DefaultAssay(Tr) <- "RNA"
  Tr <- SCTransform(Tr, variable.features.n = 100000, return.only.var.genes = FALSE)
  all_var_genes <- VariableFeatures(Tr)
  topgenes <- head(all_var_genes, 0.2*length(all_var_genes))
  length(topgenes)
  rm(all_var_genes)
  
  Tr <- RunPCA(Tr, features = topgenes)
  Tr <- RunUMAP(Tr, dims = )
...
for (node in nodes){
  ...
  new_plot <- feature_plots(Tr, ogs_gained = og_gain, ogmap = aurelia_map, node = node, bins = 16, seurat = TRUE, reduction = "tsne", color_norm = FALSE)
  plots <- append(plots, new_plot)
  
  new_plot <- feature_plots(Tr, ogs_gained = og_gain, ogmap = aurelia_map, node = node, bins = 16, seurat = TRUE, reduction = "tsne", color_norm = TRUE, min = -0.1, max = 0.25)
  plot_color_fix <- append(plot_color_fix, new_plot)
}
clusters <- DimPlot(Tr, reduction = "tsne")
...
for (node in nodes){
  ...
  heatmaps(turritopsis_ref, og_gain, node, ogmap = aurelia_map, deepfri_met, threshold = 1) 
...
pdf(file.path("/Users/oliviatatro/Downloads/surf/figures", genus, paste0(genus, "_variance.pdf")), height = 11, width = 8)
var_df <- variance(Tr, aurelia_map, nodes, genus)
dev.off()
...
#counts_mat <- GetAssayData(Tr, assay = "SCT", layer = "data")
...
plots(Nv, nematostella_map, nodes, bins = 16, "umap", nematostella_ref, genus = "nematostella")
